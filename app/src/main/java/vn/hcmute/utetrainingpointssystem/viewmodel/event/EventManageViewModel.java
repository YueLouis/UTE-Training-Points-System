...
public class EventManageViewModel extends ViewModel {
    ...
    public void registerEvent(EventRegistrationRequest request, Callback<EventRegistrationDTO> callback) {
        registrationApi.register(request).enqueue(callback);
    }

    public void loadEventDetail(Long eventId) {
        eventDetailState.setValue(new ResultState.Loading<>());
        eventApi.getEventById(eventId).enqueue(new Callback<EventDTO>() {
            @Override
            public void onResponse(Call<EventDTO> call, Response<EventDTO> response) {
                if (response.isSuccessful()) {
                    eventDetailState.setValue(new ResultState.Success<>(response.body()));
    ...
    public void deleteEvent(Long eventId, Callback<Void> callback) {
        eventApi.deleteEvent(eventId).enqueue(callback);
    }

    public void cancelEvent(Long eventId, Long studentId, final Callback<EventRegistrationDTO> callback) {
        // Fetch registration by student for this event, then cancel it
        registrationApi.getByStudent(studentId).enqueue(new Callback<List<EventRegistrationDTO>>() {
            ...
            public void onResponse(Call<List<EventRegistrationDTO>> call, Response<List<EventRegistrationDTO>> response) {
                if (response.isSuccessful() && response.body() != null) {
                    ...
                    for (EventRegistrationDTO reg : response.body()) {
                        if (reg.eventId != null && reg.eventId.equals(eventId)) {
                            // Found the registration, cancel it
                            registrationApi.cancel(reg.id).enqueue(new Callback<EventRegistrationDTO>() {
                                @Override
                    ...
                    callback.onFailure(null, new Exception("Không tìm thấy đăng ký cho sự kiện này"));
                } else {
                    callback.onFailure(null, new Exception("Lỗi tải danh sách đăng ký"));
                }
            }
