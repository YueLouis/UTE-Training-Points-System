...
public class EventCreateActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        editEventId = getIntent().getLongExtra("eventId", -1);
        isEditMode = editEventId != -1;
        ...
        if (isEditMode) {
            btnSubmit.setText("Lưu chỉnh sửa");
            loadEventData();
        } else {
    ...
    private void loadEventData() {
        if (editEventId <= 0) return;

        // Load event details from ViewModel
        vm.loadEventDetail(editEventId);
    }
    ...
    private void submitEdit() {
        ...
        vm.updateEvent(editEventId, req);
    }
